...
@inline function get_cache(cache::BitCache64, i)
    int = cache.cache
    ifelse(i == 0, int >> 24, int >> 24 + (int >> (24 - 8i)) % UInt8)
end
...
mutable struct CachedBitVector <: AbstractBitVectorRA
    ...
    function CachedBitVector(bits::BitVector)
        ...
        cache = [BitCache64() for _ = 1:n_cache]

        overall = 0
        relative = 0
        for current_short = 1:n_short
...
Base.show(io::IO, ::MIME"text/plain", z::CachedBitVector) =
    print(io, "CachedBitVector: ", z.bits)
