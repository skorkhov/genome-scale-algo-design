...
i = 9_000
@btime GSAD.rank($bv, $i);
@btime IndexableBitVectors.rank($bv0, $i);
@btime IndexableBitVectors.rank($bv1, $i);
@btime GSAD.rank($bv2, $i);
@btime GSAD.rank($bv3, $i);
...
IndexableBitVectors.rank(bv0, len)
IndexableBitVectors.rank(bv1, len)
Int(GSAD.rank(bv2, len))
...
# Experiment - are select(::BitVectorRSA) operations ~constant time? 
using Random
using GSAD
using BenchmarkTools
bitvector = bitrand(1_000_000_000)
v1 = BitVectorRSA(bitvector)
v2 = BitVectorRA(bitvector)
v3 = BitVectorSA(bitvector)
...
# Experiment - are select(::BitVectorSA) operations ~constant time? 
using Random
using GSAD
using BenchmarkTools
bitvector = bitrand(100_000_000)
v1 = BitVectorSA(bitvector)
v2 = BitVectorRSA(bitvector)
@btime BitVectorSA($bitvector);
@btime BitVectorRSA($bitvector);
...
bitvector = BitVector(rand(Float16, 10_000_000) .< 0.05)
v1 = BitVectorSA(bitvector)
v2 = BitVectorRSA(bitvector)
@btime BitVectorSA($bitvector);
@btime BitVectorRSA($bitvector);
